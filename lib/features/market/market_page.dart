    ...
    return Scaffold(
      ...
      body: RefreshIndicator(onRefresh: _load, child: ListView(padding: const EdgeInsets.fromLTRB(16,12,16,24), children: [
        ...
        ...[
          ...
          {'c':500,'p':25},
          {'c':1000,'p':30},
        ].map((e)=> Card(child: ListTile(leading: _coinIcon(), title: Text('${e['c']} Coin'), subtitle: Text('${e['p']} TL'), trailing: FilledButton(onPressed: () async { await SubscriptionService.buyCoinPack(e['c'] as int, e['p'] as int); await _load(); if(mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('${e['c']} Coin eklendi ✓'))); }, child: Text('Satın Al')),))),
        const SizedBox(height:16),
